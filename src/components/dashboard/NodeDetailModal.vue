      ...
      <div v-if="node" class="space-y-3">
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="space-y-1">
            ...
            <div class="font-mono text-sm">{{ nodeInfo.host || '-' }}:{{ nodeInfo.port || '-' }}</div>
          </div>
          <div class="space-y-1">
            <label class="text-xs text-base-content/60">网络名称</label>
            <div class="text-sm">{{ nodeInfo.networkName || '-' }}</div>
          </div>
          <div class="space-y-1">
            <label class="text-xs text-base-content/60">协议</label>
            <div class="badge badge-info badge-sm">{{ nodeInfo.protocol?.toUpperCase() }}</div>
          </div>
          <div class="space-y-1">
            <label class="text-xs text-base-content/60">最大带宽</label>
            <div class="text-sm">{{ nodeInfo.maximumBandwidth || '-' }} Mbps</div>
          </div>
          <div class="space-y-1">
          ...
          <div class="space-y-1">
            ...
            <div class="badge badge-sm" :class="nodeInfo.isRelay ? 'badge-success' : 'badge-ghost'">
              {{ nodeInfo.isRelay ? '是' : '否' }}
            </div>
          </div>
          <div class="space-y-1">
            <label class="text-xs text-base-content/60">创建时间</label>
        ...
        <div v-if="node.contact?.qq || node.contact?.email" class="space-y-2 pt-2 border-t border-base-300">
          <div v-if="node.contact?.qq" class="flex items-center gap-2">
            ...
            <span class="text-sm">{{ node.contact.qq }}</span>
          </div>
          <div v-if="node.contact?.email" class="flex items-center gap-2">
            ...
            <span class="text-sm">{{ node.contact.email }}</span>
          </div>
        </div>
      </div>
